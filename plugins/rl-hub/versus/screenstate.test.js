...
describe('screenState', () => {
  it('reports no desktop client when telemetry has never connected', () => {
    expect(screenState({ hasClient: false })).toBe(STATES.NO_CLIENT);
  });

  it('reports game closed when the client is up but nothing is running', () => {
    expect(screenState({ hasClient: true, gameState: null })).toBe(STATES.GAME_CLOSED);
  });

  it('reports pre-match when a game exists with no players yet', () => {
    expect(screenState({ hasClient: true, gameState: { players: [] } })).toBe(STATES.PRE_MATCH);
  });

  it('reports live during normal play', () => {
    expect(screenState({ hasClient: true, gameState: live })).toBe(STATES.LIVE);
  });

  it('reports replay when the replay flag is set', () => {
    expect(screenState({ hasClient: true, gameState: { ...live, is_replay: true } }))
      .toBe(STATES.REPLAY);
  });

  it('reports ended when a winner exists', () => {
    expect(screenState({ hasClient: true, gameState: { ...live, has_winner: true } }))
      .toBe(STATES.ENDED);
  });

  it('reports stale past the threshold', () => {
    expect(screenState({ hasClient: true, gameState: live, msSinceFrame: STALE_AFTER_MS + 1 }))
      .toBe(STATES.STALE);
  });
  ...
  it('prefers ended over replay', () => {
    expect(screenState({ hasClient: true, gameState: { ...live, is_replay: true, has_winner: true } }))
      .toBe(STATES.ENDED);
  });

  it('prefers ended over stale — a finished match is not a broken feed', () => {
    expect(screenState({ hasClient: true, gameState: { ...live, has_winner: true }, msSinceFrame: 99999 }))
      .toBe(STATES.ENDED);
  });

  it('prefers stale over live', () => {
    expect(screenState({ hasClient: true, gameState: live, msSinceFrame: 99999 }))
      .toBe(STATES.STALE);
  });
});
...
describe('centre band selection', () => {
  ...
  it('shows neither in a non-live state, so the empty state owns the centre', () => {
    for (const s of [STATES.NO_CLIENT, STATES.GAME_CLOSED, STATES.PRE_MATCH, STATES.ENDED, STATES.STALE]) {
